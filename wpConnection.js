    ...
    Mai.ev.on("messages.upsert", async (chatUpdate) => {
        ...
        if (chatUpdate.type == 'notify') {
            ...
            if (mek.key.fromMe) {
                ...
                if(text.toLowerCase() !== ''){
                    numero = formatarNumero.formatarNumero(mek.key.remoteJid);
                    const mensagem = await db.select('whatsapp_mensagens', '*', 'numero = ? AND enviado = 0', [numero] );
                    if(mensagem.length > 0){
                        const url = mensagem[0].arquivo; // Supõe-se que 'arquivo' seja a URL do arquivo
                        const downloadsDir = path.resolve(__dirname, 'downloads');
                        const filePath = path.resolve(downloadsDir, path.basename(url));
                
                        // Verifica se o diretório 'downloads' existe, cria se não existir
                        ...
                        await Mai.sendMessage(mek.key.remoteJid, { document: fs.readFileSync(filePath), mimetype: 'application/pdf', fileName: path.basename(url) }, { quoted: mek });
                
                        // Opcional: Excluir o arquivo após o envio
                        fs.unlinkSync(filePath);

                        const id = mensagem[0].id; // Asegure-se de que este é o ID correto da mensagem que você quer atualizar
                        const dataAtual = new Date().toISOString().slice(0, 19).replace('T', ' '); // Formata a data atual no formato YYYY-MM-DD HH:MM:SS
                        
                        await db.update(
                          'whatsapp_mensagens', 
                          { enviado: 1, data_enviado: dataAtual }, // Dados a serem atualizados
                          `id = ${id}` // Condição WHERE
                        );

                    }else{
                        io.emit('log', 'Nenhuma mensagem encontrada');
                    }
                }
